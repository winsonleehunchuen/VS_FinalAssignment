...
namespace VS_FinalAssignment
{
    public partial class adminViewFeedback : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            DataTable dt = dbcon.getDataSQL("select * from feedback;");
            int i = 1;

            table.Append("<table border='1' style='width:100%'>");
            table.Append("<tr><th>ID</th><th>Fullname</th><th>Email</th><th>Feedback</th><th>Created_at</th><th>Delete</th></tr>");

            foreach (DataRow dr in dt.Rows)
            {
                table.Append("<tr>");
                table.Append("<td>" + i + "</td><td>" + dr[1].ToString() + "</td><td>" + dr[2].ToString() + "</td><td>" + dr[3].ToString() + "</td><td>" + dr[4].ToString() + "</td>");
                table.Append("<td>" + "<a href = 'adminDeleteFeedback.aspx?id=" + dr[0] + "'><img src = 'images/delete.png' width = 22px height = 22px ></a>" + "</td>");
                table.Append("</tr>");
                i++;
            }
            table.Append("</table>");
